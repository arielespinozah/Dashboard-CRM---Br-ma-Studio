...
import { Search, Plus, Trash2, X, Package, Check, Printer, User, CreditCard, RefreshCw, Edit3, Download, Share2, Copy, Calendar, DollarSign, ChevronRight } from 'lucide-react';
...
// ... (Keep convertNumberToWordsEs function unchanged) ...
...
export const Sales = () => {
  ...
  // ... (Effects and Helper functions remain unchanged: loadData, syncSalesToCloud, handleAddItem, etc.) ...
  useEffect(() => {
  ...
  const handleDeleteSale = (id: string) => {
      ...
      if (confirm("¿Estás seguro de eliminar este recibo permanentemente?")) {
          const updatedSales = sales.filter(s => s.id !== id);
          syncSalesToCloud(updatedSales);
      }
  };

  // PDF Functions (Shortened for brevity but assumed same as before)
  ...
  return (
    <div className="space-y-6 h-full flex flex-col relative pb-safe-area">
        ...
        {view === 'list' && (
            <div className="flex-1 flex flex-col">
                                ...
                                {sales.map(sale => (
                                    <tr key={sale.id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-right flex justify-end gap-1">
                                            <button onClick={() => { setPdfSale(sale); setModalType('preview'); }} className="p-2 text-gray-400 hover:text-brand-900 hover:bg-gray-100 rounded-lg transition-colors"><Printer size={18} /></button>
                                            <button onClick={() => handleEditSale(sale)} className="p-2 text-blue-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"><Edit3 size={18} /></button>
                                            {canDelete && <button onClick={() => handleDeleteSale(sale.id)} className="p-2 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={18} /></button>}
                                        </td>
                                    </tr>
                                ))}
                ...
                <div className="md:hidden flex-1 overflow-y-auto space-y-3 pb-20">
                    {sales.map(sale => (
                        <div key={sale.id} className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm flex flex-col gap-3">
                            <div className="flex justify-between items-start">
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500 mt-0.5">{new Date(sale.date).toLocaleDateString()}</p>
                                </div>
                                <span className={`px-2 py-1 rounded-full text-[10px] font-bold ${sale.paymentStatus === 'Paid' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}>
                                    {sale.paymentStatus === 'Paid' ? 'Pagado' : 'Pendiente'}
                                </span>
                            </div>
                            <div className="flex justify-between items-center pt-2 border-t border-gray-50">
                                ...
                                <div className="flex gap-2">
                                    <button onClick={() => { setPdfSale(sale); setModalType('preview'); }} className="p-2 bg-gray-50 text-brand-900 rounded-lg"><Printer size={16}/></button>
                                    <button onClick={() => handleEditSale(sale)} className="p-2 bg-blue-50 text-blue-600 rounded-lg"><Edit3 size={16}/></button>
                                </div>
                            </div>
                        </div>
                    ))}
        ...
        {view === 'create' && (
             <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 pb-20 md:pb-0 overflow-y-auto md:overflow-visible">
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="font-bold text-gray-900 flex items-center gap-2 text-sm md:text-base"><User size={18}/> Datos del Cliente</h3>
                        ...
                        {newSale.clientName ? (
                            <div className="flex items-center gap-4 bg-gray-50 p-4 rounded-xl border border-gray-100">
                                ...
                                <div><p className="font-bold text-gray-900 text-sm">{newSale.clientName}</p><p className="text-xs text-gray-500">{newSale.clientId ? 'Cliente Registrado' : 'Cliente Casual'}</p></div>
                            </div>
                        ) : (
                            <div onClick={() => { setModalSearch(''); setIsCreatingClient(false); setModalType('client'); }} className="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center cursor-pointer hover:bg-gray-50 transition-colors group">
                                <p className="text-gray-400 font-medium group-hover:text-brand-900 transition-colors text-sm">Click para asignar un cliente</p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm min-h-[400px] flex flex-col">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="font-bold text-gray-900 flex items-center gap-2 text-sm md:text-base"><Package size={18}/> Productos & Servicios</h3>
                        ...
                        <div className="flex-1">
                            <div className="grid grid-cols-12 gap-2 pb-2 border-b border-gray-100 text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">
                                <div className="col-span-5 md:col-span-6">Desc.</div><div className="col-span-2 text-center">Cant.</div><div className="col-span-3 md:col-span-2 text-right">P.Unit</div><div className="col-span-2 text-right">Total</div>
                            </div>
                            <div className="space-y-2">
                                {newSale.items?.map((item, idx) => (
                                    <div key={idx} className="grid grid-cols-12 gap-2 items-center py-2 border-b border-gray-50 last:border-0 hover:bg-gray-50 rounded-lg px-2 -mx-2 transition-colors">
                                        <div className="col-span-5 md:col-span-6 font-medium text-gray-900 text-xs md:text-sm truncate">{item.description}</div>
                                        <div className="col-span-2 flex items-center justify-center gap-1 md:gap-2">
                                            <button onClick={() => handleUpdateQuantity(idx, -1)} className="w-5 h-5 md:w-6 md:h-6 rounded bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-600 font-bold text-xs">-</button>
                                            <span className="w-6 text-center font-medium text-gray-900 text-xs md:text-sm">{item.quantity}</span>
                                            <button onClick={() => handleUpdateQuantity(idx, 1)} className="w-5 h-5 md:w-6 md:h-6 rounded bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-600 font-bold text-xs">+</button>
                                        </div>
                                        <div className="col-span-3 md:col-span-2 text-right text-gray-600 text-xs md:text-sm">{item.unitPrice}</div>
                                        <div className="col-span-2 text-right font-bold text-gray-900 text-xs md:text-sm">{formatCurrency(item.total, settings)}</div>
                                    </div>
                                ))}
                            </div>
                        </div>
                ...
                <div className="space-y-6">
                    <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm h-fit sticky top-6">
                        <h3 className="font-bold text-gray-900 mb-6 flex items-center gap-2 text-sm md:text-base"><CreditCard size={18}/> Resumen de Pago</h3>
                        <div className="space-y-3 mb-6">
                            ...
                            <div className="pt-3 border-t border-gray-200 flex justify-between items-end"><span className="text-gray-900 font-bold">Total a Pagar</span><span className="text-2xl md:text-3xl font-bold text-brand-900 tracking-tight">{formatCurrency(newSale.total || 0, settings)}</span></div>
                        </div>
        ...
        {/* ... (Modals remain mostly unchanged but with max-width/height fixes for mobile) ... */}
        {modalType === 'preview' && pdfSale && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md p-0 md:p-4">
                <div className="bg-white md:rounded-2xl w-full max-w-4xl h-full md:h-[90vh] flex flex-col shadow-2xl overflow-hidden relative">
                    <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-900 text-white shrink-0">
                        <h3 className="font-bold flex items-center gap-2 text-sm md:text-base"><Check size={20} className="text-green-400"/> Venta Registrada</h3>
                        <div className="flex gap-2">
                             ...
                             <button onClick={handleDownloadPDF} className="p-2 bg-white/10 hover:bg-white/20 text-white rounded-lg transition-colors hidden md:block" title="Descargar PDF"><Download size={18}/></button>
                    ...
                    <div className="flex-1 overflow-auto bg-gray-100 p-2 md:p-8 flex justify-center">
                        <div className="scale-[0.5] origin-top md:scale-100 transition-transform">
                             <div ref={receiptRef} className="w-[210mm] min-h-[297mm] bg-white text-slate-800 relative font-sans shadow-2xl">
                                ...
                                {/* Rest of PDF content similar to Quotations but for Sales */}
        ...
        {modalType === 'client' && (
            <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
                <div className="bg-white rounded-2xl w-full max-w-md h-[450px] shadow-2xl animate-in zoom-in duration-200 overflow-hidden flex flex-col relative mx-4">
